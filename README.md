...
17. Exibindo Toasts
    ...
    3.  src/App.js -> import { ToastContainer } from 'react-toastify'; -> ToastContainer autoClose={3000}
    4.  src/styles/global.js
    5.  auth/sagas.js

18. Cadastro na aplicação
    1.  auth/actions.js -> export function signUpRequest(name, email, password) {
    2.  pages/SignUp/index -> import { useDispatch } from 'react-redux'; ->   const dispatch = useDispatch(); -> dispatch(signUpRequest(name, email, password)); -> import { signUpRequest } from '~/store/modules/auth/actions' -> takeLatest('@auth/SIGN_UP_REQUEST', signUp),
...
19. Requisições autenticadas.
    ...
    3.  auth/sagas.js -> api.defaults.headers.Authorization = `Bearer ${token}`;
    4.  pages/Dashboard/index.js -> import api from '~/services/api'; -> api.get('appointments'); -> Inspecionar -> Network -> erro no Appointments.
    5.  auth/sagas.js
        1.  export function setToken({payload }) {
        2.   takeLatest('persist/REHYDRATE', setToken),

20. Configurando Header
    1.  _layouts/default/styles.js
    2.  src/components/Header/index.js
    3.  api.adorable.io -> Criar avatar
    4.  yarn add react-icons
    5.  yarn add date-fns@next

21. Página de perfil
    1.  Criar pages/Profile/styles.js -> styled
    2.  pages/Profile/index.js
    3.  Form initialData={profile} onSubmit={handleSubmit}

22. Atualizando o perfil
    1.  src/store/modules/user/actions.js
    2.  src/store/modules/user/sagas.js
    3.  src/pages/Profile/index.js -> import { useDispatch, useSelector } from 'react-redux'; -> import { updateProfileRequest } from '~/store/modules/user/actions'; -> obs. substituir o console.tron.
    4.  modules/user/reducer.js
    5.  teste: Atualizar perfil. (Criar, Logar e atualizar).

23. Logout da aplicação
    1.  auth/actions.js -> export function signOut() {
    2.  auth/reducer.js
    3.  user/reducer.js
    4.  auth/sagas.js
        1.   takeLatest('@auth/SIGN_OUT', signOut),
        2.  export function signOut() {
    5.  pages/Profile/index.js -> import { signOut } from '~/store/modules/auth/actions'; ->     dispatch(signOut());
    6.  teste: Sair do site.

24. Estilização do Dashboard
    1.  Criar pages/Dashboard/styles.js
    2.  pages/Dashboard/index.js

pm save para salvar no pm2:
